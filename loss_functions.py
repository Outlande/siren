...
def sdf(model_output, gt):
    ...
    inter_constraint = torch.where((gt_sdf == -2)|(gt_sdf == 2)|(gt_sdf == 1), torch.exp(-1e2 * pred_sdf), torch.zeros_like(pred_sdf))
    ...
    return {'sdf': torch.abs(sdf_constraint).mean() * 3e3,  # 1e4      # 3e3
            'inter': inter_constraint.mean() * 0.5e2,  # 1e2                   # 1e3
            # 'free': free_constraint.mean() * 0.5e2,
            'space': space_constraint.mean() * 0.5e2,
