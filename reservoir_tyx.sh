...
python experiment_scripts/train_sdf.py --model_type=sine --point_cloud_path=${location}/${dataset}/10.xyz --batch_size=80000 \
    ...
    --camera_number=10 --camera_depth_path=${location}/${image_dataset}/depth/10.png --camera_intrinsic=${location}/${image_dataset}/ICL_gt.yaml --num_epochs=1000
...
do
    ...
    next=$[$frame+10]
    echo "frame-${next}"
    python experiment_scripts/train_sdf.py --model_type=sine --point_cloud_path=${location}/${dataset}/${next}.xyz --batch_size=80000 --checkpoint_path=logs/network/${output}/${frame}/checkpoints/model_current.pth\
